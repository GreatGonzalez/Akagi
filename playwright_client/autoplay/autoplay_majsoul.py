...
AKAGI_REACH_WAIT = _getf("AKAGI_REACH_WAIT", 2.50)
...
NAKI_CAND_WAIT = _getf("AKAGI_NAKI_CAND_WAIT", 0.15)
...
class AutoPlayMajsoul(object):
    ...
    def act(self, mjai_msg: dict) -> list[Point]:
        ...
        if mjai_msg['type'] == 'dahai' and not self.bot.self_riichi_accepted:
            ...
            random_time = random.uniform(0.8, 1.0)
            if not self.bot.last_kawa_tile:
                random_time = max(random_time, 1.0)
                try:
        ...
        if mjai_msg['type'] in ['none', 'chi', 'pon', 'daiminkan', 'ankan', 'kakan', 'hora', 'reach', 'ryukyoku', 'nukidora']:
            # Pre-waits & button waits are handled inside click_chiponkan per type
    ...
    def click_chiponkan(self, mjai_msg: dict) -> list[Point]:
        ...
        # Pre-wait: only for chi/pon/none
        _t = mjai_msg['type']
        if _t in ('chi', 'pon'):
            return_points.append(Point(-1, -1, max(0.0, NAKI_PREWAIT)))
        elif _t == 'none':
            return_points.append(Point(-1, -1, max(0.0, NAKI_NONE_PREWAIT)))
        ...
        if mjai_msg['type'] in ['chi', 'pon', 'ankan', 'kakan']:
            ...
            if mjai_msg['type'] == 'chi':
                ...
                if len(chi_candidates) == 1:
                    # No need to click, but keep timing parity
